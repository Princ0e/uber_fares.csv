...
# Load dataset
df = pd.read_csv('uber.csv')  # Make sure 'uber.csv' is in the same folder
...
# Define fixed Haversine function
def haversine(row):
    ...
    a = min(1, max(0, a))  # Clamp to avoid math domain error
    c = 2 * asin(sqrt(a)) 
    km = 6371 * c
    return km
...
print("✅ Cleaned data saved to 'uber_enhanced.csv'")
